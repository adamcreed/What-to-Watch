...
def create_movie_rankings(movies_by_id)
  ...
  movies_by_id.each_value do |movie|
    movie_rankings.push(add_movie(movie)) unless has_few_reviews?(movie)
  end
...
def add_movie(movie)
  [movie.average_rating, movie.movie_id]
end
...
def lookup_movie(movies_by_title)
  ...
  movie_list = search_movies(movies_by_title, movie_to_look_up)

  movie_list.sort.each { |title, movie| puts "#{movie}" }
  puts 'No matching titles found.' if movie_list.empty?
end

def search_movies(movies_by_title, movie_to_look_up)
  movies_by_title.select do |title, movie|
    title.downcase.include? movie_to_look_up
  end
end
...
def get_recommendations(users, movie_rankings, movies_by_id)
  ...
  print "\nHere are the top ten recommendations just for you:\n\n"
  recommendation_list_ids.each {|id| puts movies_by_id[id].title}
  puts # List formatting
end
...
def add_recommendation(user, movie, recommendations)
  recommendations.push(movie[1]) unless user_has_seen?(user, movie[1])
end
